...
//   const organizationId = getOrganizationId();
//   const cleanId = String(id).trim();  // Ensure ID is trimmed
  
//   // 1. Log the incoming data for debugging
//   console.log('Updating task with ID:', cleanId, 'and data:', data);
  
//   // 2. Filter payload to only include allowed fields, as per TASK_SCHEMA.PY assumptions
//   const allowedFields = [
//     "title",
//     "description",
//     "status",
...
//     "due_date",
//     "project_id",
//     "member_ids",  // Ensure this is an array of integers
...
//   // 3. Handle due_date formatting (ensure it's in ISO-8601 as per schema)
//   if (body.due_date && typeof body.due_date === "string" && !body.due_date.includes("T")) {
//     body.due_date = `${body.due_date}T00:00:00Z`;  // Add Z for UTC, assuming schema expects it
//   }
  
//   // 4. Add organization_id if required by schema (based on your context)
//   body.organization_id = organizationId;  // Keep this for legacy, as in your code
...
//       console.error("Server error details:", errorMessage);  // Log for debugging
//       throw new Error(errorMessage);
//     }
...
// src/api/tasks.js — updateTask (replace existing)
export async function updateTask(id, data) {
  if (!id && id !== 0) {
