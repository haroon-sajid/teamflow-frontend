...
// src/api/auth.js 
// const API_URL = import.meta.env.VITE_API_URL || "http://127.0.0.1:8000";
const API_URL = (import.meta.env.VITE_API_URL || "https://teamflow-backend-1tt9.onrender.com").trim(); // ✅ Fixed trailing spaces
...
export async function validateInvitationToken(token) {
  const res = await fetch(`${API_URL}/auth/invitations/validate/${encodeURIComponent(token)}`); // ✅ Added encoding
  const result = await res.json();
...
export async function resendInvitation(email) {
  ...
  const res = await fetch(`${API_URL}/auth/invitations/resend/${encodeURIComponent(email)}`, { // ✅ Added encoding
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
